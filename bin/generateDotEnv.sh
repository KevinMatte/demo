...
# Parameter: If 'bump' is first parameter, src/docker/demo_version.txt is bumped where the image ID has changed.
#
...
source src/docker/demo_version.txt

cat <<EOF >${1:-.env}
PROJ_ROOT=$(git rev-parse --show-toplevel)

DEMO_UI_IMAGE=demo_ui
DEMO_UI_VERSION=${DEMO_UI_VERSION}
...
DEMO_MARIADB_VERSION=${DEMO_MARIADB_VERSION}

DEMO_JAVA_VERSION=${DEMO_JAVA_VERSION}
